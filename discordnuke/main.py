...
os.system(f'cls & mode 85,20 & title [nuker] - Configuration')
...
def RichPresence():
    if rich_presence == "y" or rich_presence == "Y":
        try:
            ...
            RPC.connect() 
            RPC.update(details="Connected", large_image="averylarge2", small_image="avery", large_text="github.com/ziminl/old-files", start=time.time())
        except:
            pass
...
class Avery:
    ...
    async def Menu(self):
        os.system(f'cls & mode 85,20 & title [nuker] - Connected: {client.user}')
        print(f'''nuke''')
